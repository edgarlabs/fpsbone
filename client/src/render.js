...
function plateTexOf(tier) {
  ...
  const { band, pips } = TIERS[tier];
  const dev = deviceOf(band, pips);
  const { cv, w, h } = dev.stack ? stackTex(dev) : rowTex(dev);
 ...
 * made in two places rather than a client quirk. `deviceOf` agrees with both: a private's
 * device is zero chevrons.
 *
